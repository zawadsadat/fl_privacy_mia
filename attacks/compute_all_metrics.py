...
def get_tpr_at_fpr(fpr_arr, tpr_arr, target_fpr):
...
def main():
    ...
    for name, scores, fpr_arr, tpr_arr, auc, bal_acc in results:
        ...
        # PPV under different priors
        # Use TPR@5%FPR and FPR=5% for PPV computation
        tpr_5 = get_tpr_at_fpr(fpr_arr, tpr_arr, 0.05)
        for prior in membership_priors:
            ppv = compute_ppv(tpr_5, 0.05, prior)
            pct = f"{prior*100:.0f}"
            row[f"PPV@5%FPR_prior{pct}%"] = round(ppv, 4)
    ...
    print("-" * 55)
    for r in rows:
        print(f"{r['Attack']:<22} {r['PPV@5%FPR_prior1%']:>7.4f} {r['PPV@5%FPR_prior5%']:>7.4f} "
              f"{r['PPV@5%FPR_prior10%']:>8.4f} {r['PPV@5%FPR_prior50%']:>8.4f}")
