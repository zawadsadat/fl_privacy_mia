...
Attack features per sample:
  - Gradient norm (L2)
  - Gradient norm (L1)
  - Per-layer gradient norms (5 layers)
...
def main():
    ...
    with torch.no_grad():
        ...
        preds = model(X_test_t).squeeze(-1)
    ...
    np.save(f"experiments/whitebox_features{suffix}.npy", X_attack)
    np.save(f"experiments/whitebox_labels{suffix}.npy", y_attack)
