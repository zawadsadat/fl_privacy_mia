"""
Federated Membership Privacy Audit Score (FMPAS)

For each FL/DP configuration, computes:
    Risk = max over all attacks and FPR thresholds of (TPR - FPR)

Then recommends the smallest DP noise level (sigma) that keeps
Risk below a target threshold while preserving acceptable accuracy.

Inputs:
  - experiments/metrics_*_nodp.csv and metrics_*_dp.csv
    OR runs the full sweep itself using saved epsilon sweep results

  - experiments/epsilon_sweep_results.csv (from run_epsilon_sweep.py)

Outputs:
  - experiments/privacy_audit_scores.csv
  - experiments/plots/audit_score_recommendation.png
